...
import { LayoutDashboard, Store, Package, ClipboardList, Star, TrendingUp, Bell, Settings, LogOut, ShoppingBag, Menu, X } from 'lucide-react';
      ...
      <div className="p-5 border-b border-gray-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">SELLER DASHBOARD</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-900 truncate">{profile?.full_name || 'Seller'}</p>
        ...
        {navItems.map(({ name, icon: Icon, href }) => (
          <Link key={name} href={href} onClick={() => setIsMobileOpen(false)}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              isActive(href) ? 'bg-orange-50 text-orange-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
            }`}>
    ...
    <>
      <div className="lg:hidden fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-30 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-900 text-sm">Seller Dashboard</span>
        </div>
