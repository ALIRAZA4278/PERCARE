...
export default function ReportPetModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({ petName: '', species: '', breed: '', location: '', contactNumber: '', description: '' });

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Report submitted:', { ...formData, type: reportType });
    onClose();
    setFormData({ petName: '', species: '', breed: '', location: '', contactNumber: '', description: '' });
    setReportType('lost');
  };
        ...
        <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="p-5 sm:p-6">
            ...
            <div className="mb-4">
              ...
              <input type="text" name="petName" value={formData.petName} onChange={handleChange} placeholder={reportType === 'lost' ? 'Enter pet name' : 'Unknown or name if known'} required className={inputClass} />
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Species <span className="text-red-500">*</span></label>
                <input type="text" name="species" value={formData.species} onChange={handleChange} placeholder="Dog, Cat, Bird..." required className={inputClass} />
              </div>
              <div>
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">{reportType === 'lost' ? 'Last Seen Location' : 'Found Location'} <span className="text-red-500">*</span></label>
              <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="Area, city" required className={inputClass} />
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Contact Number <span className="text-red-500">*</span></label>
              <input type="tel" name="contactNumber" value={formData.contactNumber} onChange={handleChange} placeholder="+92 3XX XXXXXXX" required className={inputClass} />
            </div>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Description</label>
              <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Color, distinguishing features, collar details..." rows={4} className={`${inputClass} resize-none py-3`} />
            </div>
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3.5 rounded-lg transition-colors shadow-sm">Submit Report</button>
          </form>
        </div>
      </div>
    </>
