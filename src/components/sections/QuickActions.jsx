    ...
    <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 px-4">
      {actions.map((action, index) => (
        <Link
          key={index}
          href={action.href}
          className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200 hover:shadow-lg transition-all cursor-pointer group flex items-center justify-between gap-3"
        >
          <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
            <div className={`w-12 h-12 sm:w-14 sm:h-14 ${action.bgColor} rounded-lg sm:rounded-xl flex items-center justify-center shrink-0`}>
              {action.icon ? (
