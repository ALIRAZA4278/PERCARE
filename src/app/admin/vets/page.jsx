  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Veterinarians</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {paged.length === 0 ? (
                ...
                <tr key={vet.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center text-gray-400 font-bold text-xs shrink-0 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        <p className="text-white font-medium text-xs truncate max-w-[120px]">{vet.user?.full_name || '—'}</p>
                        <p className="text-gray-500 text-[10px] truncate max-w-[120px]">{vet.user?.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden sm:table-cell">{vet.license_number || '—'}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden md:table-cell">{vet.specialization || '—'}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden lg:table-cell">{vet.experience_years ? `${vet.experience_years} yrs` : '—'}</td>
                  <td className="px-4 py-3 hidden sm:table-cell">
                    {vet.certificate_url
                      ? <a href={vet.certificate_url} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 inline-flex items-center gap-1 text-xs"><ExternalLink size={12} /> View</a>
                      : <span className="text-gray-600 text-xs">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${vet.is_approved ? 'bg-green-950 text-green-400' : 'bg-orange-950 text-orange-400'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-gray-900 rounded-2xl w-full max-w-lg border border-gray-800 p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-white">{modal.mode === 'edit' ? 'Edit Vet Profile' : 'Add Vet Profile'}</h3>
              ...
              <div className="space-y-3">
                {modal.mode === 'add' && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 mb-1.5">Search User</label>
