...
const statusBadge = {
  ...
  reunited: 'bg-vitality/10 text-vitality',
        ...
        <div className="flex gap-2 flex-wrap">
          {STATUS_TABS.map(s => (
            <button key={s} onClick={() => setTab(s)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold capitalize transition-colors flex items-center gap-1.5 ${tab === s ? 'bg-primary text-white' : 'bg-card text-muted-foreground border border-border hover:border-border'}`}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-muted text-muted-foreground uppercase">{post.type}</span>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusBadge[post.status] || 'bg-muted text-muted-foreground'}`}>{post.status}</span>
                  <span className="text-xs text-muted-foreground capitalize">{post.species}{post.breed ? ` · ${post.breed}` : ''}</span>
                </div>
                <p className="text-foreground font-semibold">{post.pet_name || 'Unknown Pet'}</p>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={() => setDeleteModal(post)} disabled={processing === post.id}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-muted hover:bg-emergency/10 text-emergency text-xs font-semibold rounded-lg transition-colors border border-border disabled:opacity-50">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-sm border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-foreground">Remove Post</h3>
