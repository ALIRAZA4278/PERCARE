...
export default function ApprovalsPage() {
  ...
  const btnA = "flex items-center gap-1.5 px-3 py-1.5 bg-vitality hover:bg-vitality/90 text-white text-xs font-semibold rounded-lg transition-colors";
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        {TABS.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap flex items-center gap-2 transition-colors ${activeTab === tab ? 'bg-primary text-white' : 'bg-card text-muted-foreground border border-border hover:border-border'}`}>
            {tab}
            {counts[tab] > 0 && <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab ? 'bg-card/20' : 'bg-primary/10 text-primary'}`}>{counts[tab]}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {activeTab === 'Vets' && (vets.length === 0 ? <Empty /> : vets.map(vet => (
          <Card key={vet.id} actions={[
            ...
            <p className="font-bold text-foreground">{vet.user?.full_name || 'Unknown'}</p>
            <p className="text-xs text-muted-foreground mb-2">{vet.user?.email}</p>
            ...
            {vet.certificate_url && <a href={vet.certificate_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline mt-2"><ExternalLink size={12} /> View Certificate</a>}
          </Card>
        )))}
        ...
        {activeTab === 'Clinics' && (clinics.length === 0 ? <Empty /> : clinics.map(clinic => (
          <Card key={clinic.id} actions={[
            ...
            <p className="text-xs text-muted-foreground mb-2">Owner: {clinic.owner?.full_name || '—'} · {clinic.owner?.email || '—'}</p>
            <div className="flex flex-wrap gap-1.5">
        ...
        {activeTab === 'Stores' && (stores.length === 0 ? <Empty /> : stores.map(store => (
          <Card key={store.id} actions={[
            ...
            <p className="text-xs text-muted-foreground mb-2">Owner: {store.owner?.full_name} · {store.owner?.email}</p>
            <div className="flex flex-wrap gap-1.5">
        ...
        {activeTab === 'Shelters' && (shelters.length === 0 ? <Empty /> : shelters.map(shelter => (
          <Card key={shelter.id} actions={[
            ...
            <p className="font-bold text-foreground">{shelter.name}</p>
            <p className="text-xs text-muted-foreground mb-2">{shelter.city}{shelter.address ? ` · ${shelter.address}` : ''}</p>
            <div className="flex flex-wrap gap-1.5">
          ...
          <Card key={product.id} actions={[
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Store: {product.store?.name} · Rs. {Number(product.price).toLocaleString()}</p>
              </div>
            </div>
          </Card>
        )))}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-sm border border-border p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-foreground">Reject — Add Reason</h3>
                <button onClick={() => { setRejectModal(null); setRejectReason(''); }} className="p-1 hover:bg-muted rounded-lg text-muted-foreground"><X size={18} /></button>
              </div>
