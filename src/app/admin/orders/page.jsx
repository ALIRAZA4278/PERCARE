    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{orders.length} total · Rs. {totalRevenue.toLocaleString()} delivered revenue</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 hidden sm:table-cell">Total</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="text-center text-muted-foreground py-12">No orders found</td></tr>
              ) : filtered.map(order => {
                  ...
                  <>
                    <tr key={order.id}
                      ...
                      <td className="px-4 py-3">
                        <p className="text-foreground font-medium text-xs">{order.buyer?.full_name || '—'}</p>
                        <p className="text-muted-foreground text-[10px] hidden sm:block">{order.buyer?.email}</p>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground/40 text-xs hidden sm:table-cell">Rs. {Number(order.total_amount || 0).toLocaleString()}</td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${order.payment_status === 'paid' ? 'bg-vitality/10 text-vitality' : 'bg-amber/10 text-amber'}`}>
                          {order.payment_status || 'pending'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusBadge[order.status] || 'bg-muted text-muted-foreground'}`}>
                          {order.status}
                        </span>
                      </td>
                    ...
                    {expanded === order.id && (
                      <tr key={`${order.id}-detail`} className="border-b border-border bg-muted">
                        <td colSpan={7} className="px-6 py-5" onClick={e => e.stopPropagation()}>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {/* Items */}
                            <div>
                              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Items ({order.items?.length || 0})</p>
                                ...
                                <div className="space-y-2">
                                  {order.items.map((item, i) => (
                                    <div key={i} className="flex items-center gap-2.5">
                                      <div className="w-7 h-7 bg-muted rounded shrink-0 overflow-hidden">
                                      ...
                                      <div className="flex-1 min-w-0">
                                        <p className="text-xs text-foreground truncate">{item.product?.name || 'Unknown'}</p>
                                        <p className="text-[10px] text-muted-foreground">×{item.quantity} · Rs. {Number(item.unit_price || 0).toLocaleString()}</p>
                                      </div>
                                    </div>
                                  ))}
                                </div>
                              ...
                              {order.shipping_address && (
                                <div className="mt-3 pt-3 border-t border-border">
                                  <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-1">Delivery</p>
                                  <p className="text-xs text-muted-foreground">{order.shipping_address}{order.shipping_city ? `, ${order.shipping_city}` : ''}</p>
