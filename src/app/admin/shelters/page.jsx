  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Shelters</h1>
          <p className="text-sm text-muted-foreground mt-1">{shelters.length} total · {counts.verified} verified · {counts.unverified} unverified</p>
        </div>
        ...
        <div className="flex gap-2">
          {['all', 'verified', 'unverified'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold capitalize transition-colors flex items-center gap-1.5 ${filter === f ? 'bg-primary text-white' : 'bg-card text-muted-foreground border border-border hover:border-border'}`}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3">Shelter</th>
                ...
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 hidden lg:table-cell">Contact</th>
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 hidden lg:table-cell">Website</th>
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={7} className="text-center text-muted-foreground py-12 text-sm">No shelters found</td></tr>
              ) : paged.map(shelter => (
                <tr key={shelter.id} className="border-b border-border last:border-0 hover:bg-muted transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p className="text-foreground font-medium text-xs truncate max-w-[130px]">{shelter.name}</p>
                        <p className="text-muted-foreground text-[10px]">{shelter.created_at ? new Date(shelter.created_at).toLocaleDateString() : ''}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell">
                    <p className="text-xs text-foreground truncate max-w-[110px]">{shelter.owner?.full_name || '—'}</p>
                    <p className="text-[10px] text-muted-foreground truncate max-w-[110px]">{shelter.owner?.email}</p>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <p className="text-xs text-foreground">{shelter.city || '—'}</p>
                    {shelter.country && <p className="text-[10px] text-muted-foreground">{shelter.country}</p>}
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <p className="text-xs text-muted-foreground">{shelter.phone || '—'}</p>
                    {shelter.email && <p className="text-[10px] text-muted-foreground truncate max-w-[130px]">{shelter.email}</p>}
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    {shelter.website
                      ? <a href={shelter.website} target="_blank" rel="noreferrer" className="text-primary hover:text-primary inline-flex items-center gap-1 text-xs"><ExternalLink size={12} /> Visit</a>
                      : <span className="text-muted-foreground text-xs">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${shelter.is_verified ? 'bg-vitality/10 text-vitality' : 'bg-amber/10 text-amber'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-lg border border-border p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">{modal.mode === 'edit' ? 'Edit Shelter' : 'Add Shelter'}</h3>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-muted-foreground mb-1.5">Shelter Name</label>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={key} className="flex items-center gap-2 p-2.5 bg-muted rounded-xl">
                      ...
                      <label htmlFor={key} className="text-xs font-medium text-foreground cursor-pointer leading-tight">{label}</label>
                    </div>
                  ))}
                </div>
              </div>
