  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Clinics & Hospitals</h1>
          <p className="text-sm text-muted-foreground mt-1">{clinics.length} total · {counts.approved} approved · {counts.pending} pending · {counts.hospital} emergency-capable</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 hidden sm:table-cell">City</th>
                <th className="text-left text-xs font-semibold text-muted-foreground px-4 py-3 hidden md:table-cell">Contact</th>
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={6} className="text-center text-muted-foreground py-12 text-sm">No listings found</td></tr>
              ) : paged.map(clinic => (
                <tr key={clinic.id} className="border-b border-border last:border-0 hover:bg-muted transition-colors">
                  <td className="px-4 py-3">
                    <p className="text-foreground font-medium text-xs truncate max-w-[160px]">{clinic.name}</p>
                    <p className="text-muted-foreground text-[10px] truncate max-w-[160px]">{clinic.owner?.full_name || 'No owner linked'}</p>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground text-xs hidden sm:table-cell">{clinic.city || '—'}</td>
                  <td className="px-4 py-3 text-muted-foreground text-xs hidden md:table-cell">{clinic.phone || clinic.email || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${clinic.is_emergency_available ? 'bg-emergency/10 text-emergency' : 'bg-primary/10 text-primary'}`}>
                      {clinic.is_emergency_available ? 'Hospital' : 'Clinic'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${clinic.is_approved ? 'bg-vitality/10 text-vitality' : 'bg-amber/10 text-amber'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-lg border border-border p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-foreground">{modal.mode === 'edit' ? 'Edit Listing' : 'Add Listing'}</h3>
              ...
              <div className="space-y-3">
                {modal.mode === 'add' && (
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground mb-1.5">Owner (optional)</label>
                ...
                <div className="flex items-center gap-3 p-3 bg-muted rounded-xl">
                  ...
                  <label htmlFor="is_emergency_available" className="text-sm font-medium text-foreground cursor-pointer">24/7 Emergency (list as Hospital)</label>
                </div>

                <div className="flex items-center gap-3 p-3 bg-muted rounded-xl">
                  <input type="checkbox" id="is_approved" checked={form.is_approved} onChange={e => setForm(f => ({ ...f, is_approved: e.target.checked }))}
                    className="w-4 h-4 accent-green-500" />
                  <label htmlFor="is_approved" className="text-sm font-medium text-foreground cursor-pointer">Mark as Approved</label>
                </div>
              </div>
