  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Clinics & Hospitals</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {paged.length === 0 ? (
                ...
                <tr key={clinic.id} className="border-b border-gray-200 last:border-0 hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <p className="text-gray-900 font-medium text-xs truncate max-w-[160px]">{clinic.name}</p>
                    <p className="text-gray-500 text-[10px] truncate max-w-[160px]">{clinic.owner?.full_name || 'No owner linked'}</p>
                  </td>
                  <td className="px-4 py-3 text-gray-500 text-xs hidden sm:table-cell">{clinic.city || '—'}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs hidden md:table-cell">{clinic.phone || clinic.email || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${clinic.is_emergency_available ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
                      {clinic.is_emergency_available ? 'Hospital' : 'Clinic'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${clinic.is_approved ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-lg border border-gray-200 p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-gray-900">{modal.mode === 'edit' ? 'Edit Listing' : 'Add Listing'}</h3>
              ...
              <div className="space-y-3">
                {modal.mode === 'add' && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5">Owner (optional)</label>
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <label htmlFor="is_emergency_available" className="text-sm font-medium text-gray-900 cursor-pointer">24/7 Emergency (list as Hospital)</label>
                </div>
