          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {paged.length === 0 ? (
                ...
                <tr key={product.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-9 h-9 bg-gray-800 rounded-lg flex items-center justify-center shrink-0 overflow-hidden">
                        {product.image_url
                          ? <img src={product.image_url} alt="" className="w-full h-full object-cover rounded-lg" />
                          : <span>📦</span>}
                      </div>
                      <div className="min-w-0">
                        <p className="text-white font-medium text-xs truncate max-w-[120px]">{product.name}</p>
                        {product.brand && <p className="text-gray-500 text-[10px] truncate max-w-[120px]">{product.brand}</p>}
                        {product.is_medicine && <span className="text-[9px] font-bold text-purple-400 bg-purple-950 px-1.5 py-0.5 rounded">Medicine</span>}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden sm:table-cell">{product.store?.name || '—'}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden md:table-cell">{product.category?.name || '—'}</td>
                  <td className="px-4 py-3">
                    <p className="text-white font-semibold text-xs">Rs. {Number(product.price || 0).toLocaleString()}</p>
                    {product.sale_price && <p className="text-green-400 text-[10px]">Sale: Rs. {Number(product.sale_price).toLocaleString()}</p>}
                  </td>
                  <td className="px-4 py-3 text-xs hidden md:table-cell">
                    {product.stock_quantity != null
                      ? <span className={product.stock_quantity < 5 ? 'text-red-400 font-semibold' : 'text-gray-400'}>{product.stock_quantity}</span>
                      : <span className="text-gray-600">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    {!product.is_active
                      ? <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-gray-800 text-gray-500">Inactive</span>
                      : product.is_approved
                        ? <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-950 text-green-400">Approved</span>
                        : <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-orange-950 text-orange-400">Pending</span>}
                  </td>
                  <td className="px-4 py-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={key} className="flex items-center gap-2 p-2.5 bg-gray-800 rounded-xl">
                      ...
                      <label htmlFor={key} className="text-xs font-medium text-white cursor-pointer">{label}</label>
                    </div>
                  ))}
                </div>
              </div>
