...
export default function SellerSettingsPage() {
  ...
  const items = [
    { icon: Bell, label: 'Push Notifications', desc: 'Reminders, updates & alerts', type: 'toggle', value: pushNotifications, onChange: () => setPushNotifications(!pushNotifications), iconBg: 'bg-orange-50', iconColor: 'text-orange-600' },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/seller-dashboard" className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ArrowLeft size={18} className="text-gray-700" /></Link>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 divide-y divide-gray-100">
          {items.map(({ icon: Icon, label, desc, type, value, onChange, iconBg, iconColor }) => (
            <div key={label} className="flex items-center justify-between p-4 sm:p-5 hover:bg-gray-50 transition-colors cursor-pointer">
              ...
              {type === 'toggle' ? (
                <button onClick={onChange} className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${value ? 'bg-orange-500' : 'bg-gray-300'}`}>
