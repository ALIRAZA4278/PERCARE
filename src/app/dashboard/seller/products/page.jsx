    ...
    <div className="p-4 sm:p-6 lg:p-8">
          ...
          {['All', ...categoryOptions].map((c) => (
            <button key={c} onClick={() => setActiveCategory(c)}
              className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors whitespace-nowrap ${
                activeCategory === c ? 'bg-primary text-white' : 'bg-card text-foreground border border-border hover:bg-muted'
              }`}>{c}</button>
          ))}
        </div>
      </div>
          ...
          return (
            <div key={product.id} className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-5 border border-border flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{product.brand || 'General'} · {product.total_sold || 0} sold</p>
              </div>
              <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                <div className="text-right hidden sm:block">
                  <p className="font-bold text-foreground text-sm">Rs. {product.price?.toLocaleString()}</p>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-bold text-foreground mb-2">No products</h3>
            ...
            <div className="bg-card rounded-2xl w-full max-w-xs shadow-elevated">
              <div className="relative">
                ...
                <div className="bg-muted rounded-t-2xl h-40 flex items-center justify-center">
                  {viewProduct.image_url ? <img src={viewProduct.image_url} alt="" className="w-full h-full object-cover rounded-t-2xl" /> : <Package size={48} className="text-muted-foreground/40" />}
                </div>
              </div>
              <div className="p-5">
                ...
                {viewProduct.description && <div><p className="text-xs text-primary">Description</p><p className="text-sm text-foreground">{viewProduct.description}</p></div>}
              </div>
            </div>
          </div>
        </>
                  ...
                  <div onClick={() => fileRef.current?.click()}
                    ...
                    {imagePreview ? (
                      ...
                      <>
                        ...
                        <p className="text-[10px] text-muted-foreground/40">JPG, PNG up to 5MB</p>
                      </>
                    )}
                  </div>
                </div>
