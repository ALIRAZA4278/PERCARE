...
export default function SellerSettingsPage() {
  ...
  const items = [
    { icon: Bell, label: 'Push Notifications', desc: 'Reminders, updates & alerts', type: 'toggle', value: pushNotifications, onChange: () => setPushNotifications(!pushNotifications), iconBg: 'bg-amber/10', iconColor: 'text-amber' },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/seller" className="p-2 hover:bg-muted rounded-lg transition-colors"><ArrowLeft size={18} className="text-foreground" /></Link>
        ...
        <div className="rounded-2xl bg-card shadow-card divide-y divide-border">
          {items.map(({ icon: Icon, label, desc, type, value, onChange, iconBg, iconColor }) => (
            <div key={label} className="flex items-center justify-between p-4 sm:p-5 hover:bg-muted transition-colors cursor-pointer">
              ...
              {type === 'toggle' ? (
                <button onClick={onChange} className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${value ? 'bg-amber' : 'bg-muted-foreground/30'}`}>
