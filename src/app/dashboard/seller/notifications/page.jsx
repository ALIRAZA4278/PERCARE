...
import { Calendar, Package, Star, AlertTriangle, CheckCircle, Bell } from 'lucide-react';
import { useState, useEffect } from 'react';
import { useAuth } from '@/context/AuthContext';
import { supabase } from '@/lib/supabase';

const iconMap = {
  order: { icon: Package, iconBg: 'bg-vitality/10', iconColor: 'text-vitality', borderColor: 'border-vitality/20' },
  review: { icon: Star, iconBg: 'bg-amber/10', iconColor: 'text-amber', borderColor: 'border-amber/20' },
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Stay updated on your store activity</p>
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead} className="text-amber font-medium text-sm hover:text-amber">Mark all read</button>
        )}
      </div>
