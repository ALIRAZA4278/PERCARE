...
const statusColors = {
  Healthy: 'bg-vitality/10 text-vitality border-vitality/20',
  'Follow-up': 'bg-card text-foreground border-border',
  Recovering: 'bg-amber/10 text-amber border-amber/20',
  'Under Treatment': 'bg-card text-foreground border-border',
};

const avatarColors = ['bg-primary/10 text-primary', 'bg-primary/10 text-primary', 'bg-vitality/10 text-vitality', 'bg-amber/10 text-amber', 'bg-emergency/10 text-emergency'];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">View and manage pet medical records</p>
      </div>
              ...
              <button onClick={() => setExpandedId(isExpanded ? null : patient.id)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <h3 className="font-bold text-foreground text-sm sm:text-base">{patient.name}</h3>
                    {isVaccinated && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-vitality/10 text-vitality">Vaccinated</span>
                    )}
                  </div>
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    {patient.breed || patient.species} · {patient.species} · {formatAge(patient.age_years, patient.age_months)}
                  </p>
                  <p className="text-xs text-muted-foreground">Owner: {patient.owner?.full_name || '—'}</p>
                </div>
                ...
                <div className="border-t border-border p-4 sm:p-5 bg-muted">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-foreground">{patient.breed || '—'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-0.5">Age</p>
                      <p className="text-sm font-semibold text-foreground">{formatAge(patient.age_years, patient.age_months)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-0.5">Species</p>
                      <p className="text-sm font-semibold text-foreground capitalize">{patient.species}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-0.5">Weight</p>
                      <p className="text-sm font-semibold text-foreground">{patient.weight_kg ? `${patient.weight_kg} kg` : '—'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-0.5">Last Visit</p>
                      <p className="text-sm font-semibold text-foreground">{patient.last_visit ? new Date(patient.last_visit + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</p>
                    </div>
                    <div>
                  ...
                  <div className="mb-4">
                    <p className="text-xs text-muted-foreground mb-0.5">Owner Contact</p>
                    <p className="text-sm font-semibold text-foreground">{patient.owner?.full_name || '—'} {patient.owner?.phone ? `— ${patient.owner.phone}` : ''}</p>
                  </div>

                  {patient.medical_notes && (
                    <div className="mb-4">
                      <p className="text-xs text-muted-foreground mb-0.5">Notes</p>
                      <p className="text-sm text-foreground">{patient.medical_notes}</p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => openHistory(patient.id, patient.name)}
                      className="bg-card hover:bg-muted text-foreground font-medium px-4 py-2 rounded-lg transition-colors border border-border text-sm flex items-center gap-1.5">
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground">Patients will appear here after appointments.</p>
        </div>
      )}
      ...
      {showAddNote && (
        <>
          <div className="fixed inset-0 bg-foreground/40 z-40" onClick={() => setShowAddNote(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-sm shadow-elevated">
              <div className="flex items-center justify-between p-5 border-b border-border">
                <h2 className="text-lg font-bold text-foreground">Add Treatment Note — {notePetName}</h2>
                <button onClick={() => setShowAddNote(false)} className="p-1 hover:bg-muted rounded-lg"><X size={20} className="text-foreground" /></button>
              </div>
              <div className="p-5 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Treatment / Diagnosis</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Prescription</label>
      ...
      {showHistory && (
        <>
          <div className="fixed inset-0 bg-foreground/40 z-40" onClick={() => setShowHistory(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-md shadow-elevated max-h-[80vh] overflow-y-auto">
              <div className="flex items-center justify-between p-5 border-b border-border sticky top-0 bg-card rounded-t-2xl z-10">
                <h2 className="text-lg font-bold text-foreground">Treatment History — {historyPetName}</h2>
                <button onClick={() => setShowHistory(false)} className="p-1 hover:bg-muted rounded-lg"><X size={20} className="text-foreground" /></button>
              </div>
              <div className="p-5">
                {historyNotes.length > 0 ? (
                    ...
                    <div className="space-y-4">
                      {historyNotes.map((note) => (
                        <div key={note.id} className="relative pl-10">
                          <div className="absolute left-2.5 top-1.5 w-3 h-3 rounded-full bg-card border-2 border-border" />
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="font-semibold text-foreground text-sm">{note.treatment}</p>
                              <p className="text-xs text-muted-foreground">Prescription: {note.prescription || 'N/A'}</p>
                              <p className="text-xs text-muted-foreground">By: {note.vet?.user?.full_name || 'Dr.'}</p>
                            </div>
                  ...
                  <p className="text-sm text-muted-foreground text-center py-8">No treatment history yet</p>
                )}
              </div>
            </div>
          </div>
