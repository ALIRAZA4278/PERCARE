...
const statusColors = {
  available: 'bg-vitality/10 text-vitality',
  adopted: 'bg-primary/10 text-primary',
  foster: 'bg-amber/10 text-amber',
  medical: 'bg-emergency/10 text-emergency',
  quarantine: 'bg-amber/10 text-amber',
};
...
export default function ShelterAnimalsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex gap-2 flex-wrap">
          {['All', 'Available', 'Adopted', 'Foster', 'Medical'].map(f => (
            <button key={f} onClick={() => setFilterStatus(f)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterStatus === f ? 'bg-vitality text-white' : 'bg-card border border-border text-foreground hover:bg-muted'}`}>{f}</button>
          ))}
        </div>
      </div>
          ...
          <div key={animal.id} className="rounded-xl bg-card shadow-card overflow-hidden hover:shadow-card-hover transition-shadow">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-bold text-foreground">{animal.name}</p>
                  <p className="text-xs text-muted-foreground">{animal.species}{animal.breed ? ` · ${animal.breed}` : ''}</p>
                </div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusColors[animal.adoption_status] || 'bg-muted text-muted-foreground'}`}>{animal.adoption_status}</span>
              </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16"><div className="text-6xl mb-4">🐾</div><h3 className="text-xl font-bold text-foreground mb-2">No animals found</h3><p className="text-muted-foreground">Add animals to your shelter to manage them here.</p></div>
      )}

      {showModal && (
        <>
          <div className="fixed inset-0 bg-foreground/40 z-40" onClick={() => setShowModal(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl w-full max-w-md shadow-elevated max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between p-5 border-b border-border">
                <h2 className="text-lg font-bold text-foreground">{editAnimal ? 'Edit Animal' : 'Add Animal'}</h2>
                <button onClick={() => setShowModal(false)} className="p-1 hover:bg-muted rounded-full"><X size={18} className="text-foreground" /></button>
              </div>
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} placeholder="About this animal..."
                    className="w-full px-4 py-2.5 rounded-xl border border-border outline-none focus:border-primary text-sm text-foreground bg-card placeholder:text-muted-foreground resize-none" />
                </div>
                <div>
                  ...
                  <label className="flex items-center justify-center gap-2 w-full py-2.5 border-2 border-dashed border-border rounded-xl cursor-pointer hover:border-primary/40 transition-colors text-sm text-muted-foreground">
