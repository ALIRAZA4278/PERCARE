...
export default function ShelterSettingsPage() {
  ...
  const items = [
    { icon: Bell, label: 'Push Notifications', desc: 'Reminders, updates & alerts', type: 'toggle', value: pushNotifications, onChange: () => setPushNotifications(!pushNotifications), iconBg: 'bg-vitality/10', iconColor: 'text-vitality' },
    { icon: Moon, label: 'Dark Mode', desc: 'Switch appearance', type: 'toggle', value: darkMode, onChange: () => setDarkMode(!darkMode), iconBg: 'bg-muted', iconColor: 'text-muted-foreground' },
    { icon: Globe, label: 'Language', desc: 'English (US)', type: 'link', iconBg: 'bg-primary/10', iconColor: 'text-primary' },
    { icon: HelpCircle, label: 'Help & Support', desc: 'FAQs and contact', type: 'link', iconBg: 'bg-primary/10', iconColor: 'text-primary' },
    { icon: Info, label: 'About PetCare', desc: 'Version 1.0.0', type: 'link', iconBg: 'bg-muted', iconColor: 'text-muted-foreground' },
  ];

  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/shelter" className="p-2 hover:bg-muted rounded-lg transition-colors"><ArrowLeft size={18} className="text-foreground" /></Link>
          ...
          {items.map(({ icon: Icon, label, desc, type, value, onChange, iconBg, iconColor }) => (
            <div key={label} className="flex items-center justify-between p-4 sm:p-5 hover:bg-muted transition-colors cursor-pointer">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div><p className="font-semibold text-foreground text-sm sm:text-base">{label}</p><p className="text-xs sm:text-sm text-muted-foreground">{desc}</p></div>
              </div>
              {type === 'toggle' ? (
                <button onClick={onChange} className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${value ? 'bg-vitality' : 'bg-muted-foreground/30'}`}>
