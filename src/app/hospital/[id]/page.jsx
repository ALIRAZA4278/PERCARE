  ...
  if (!clinic) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <p className="text-xl font-bold text-gray-700 mb-4">Hospital not found</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6 pb-2">
        ...
        <div className="flex gap-2 mb-5 overflow-hidden rounded-xl">
          {['🏥', '🚑'].map((emoji, i) => (
            <div key={i} className={`flex-1 h-36 sm:h-48 flex items-center justify-center overflow-hidden rounded-xl bg-gray-200`}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        {(clinic.phone || clinic.website || clinic.opening_time) && (
          <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-200 mb-4">
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-2.5 text-sm text-gray-700">
                <Clock size={15} className="text-gray-400" /><span>{formatHours(clinic)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {services.length > 0 && (
          <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-200 mb-4">
            <h3 className="text-lg font-bold text-gray-900 mb-3">Departments & Services</h3>
            <div className="flex flex-wrap gap-2">
              {services.map((s) => (
                <span key={s} className="bg-blue-50 text-blue-700 text-sm font-medium px-4 py-2 rounded-full border border-blue-100">{s}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {vets.length > 0 && (
          <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-200 mb-4">
            ...
            <div className="space-y-2">
              {vets.map((vet) => (
                <Link key={vet.id} href={`/vets/${vet.id}`} className="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:bg-gray-50 transition-colors group">
